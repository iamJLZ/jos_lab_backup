...
#define STH(N) SETGATE(idt[T_##N],0,GD_KT,handler_##N,0);
#define STHN(N) STH(N)
	STHN(DIVIDE)
	STHN(DEBUG)
	STHN(NMI)
	...
	SETGATE(idt[T_OFLOW],0,GD_KT,handler_OFLOW,3);
	SETGATE(idt[T_BOUND],0,GD_KT,handler_BOUND,3);
	STHN(ILLOP)
	STHN(DEVICE)
	STH(DBLFLT)
	STH(TSS)
...
trap_init_percpu(void)
{
	...
	struct Taskstate* tts=&(thiscpu->cpu_ts);
	int i=cpunum();
	tts->ts_esp0 = KSTACKTOP-i*(KSTKSIZE+KSTKGAP);
	tts->ts_ss0 = GD_KD;
	tts->ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3)+i] = SEG16(STS_T32A, (uint32_t) (tts),
...
trap_dispatch(struct Trapframe *tf)
{
	...
	case T_SYSCALL:
		...
		pr->reg_eax=syscall(pr->reg_eax,pr->reg_edx,pr->reg_ecx,pr->reg_ebx,pr->reg_edi,pr->reg_esi);
		return;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if((tf->tf_cs&3)==0)
		panic("page_fault_handler:kernel fault\n");
