...
// static void hboot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm);
...
boot_alloc(uint32_t n)
{
	...
	static char* memend;
	if(!memend){
		memend=nextfree+PGSIZE*npages;
	}
	if(memend-nextfree<n){
		panic("Out of memory!");
	}
	else{
		result=nextfree;
...
mem_init(void)
{
	...
	// if(!((edxp>>3)&1))
	// 	panic("This cpu does not support large pages.");
	// large_page_enabled=1;
	uint32_t cr0;
	size_t n;
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	// hboot_map_region(kern_pgdir,KERNBASE,0xffffffff-KERNBASE,0,PTE_W);
...
#define mark_free(PN) do{\
	pages[PN].pp_ref=0;\
	pages[PN].pp_link=page_free_list;\
	page_free_list=&pages[PN];}while (0)
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	size_t mpg=MPENTRY_PADDR/PGSIZE;
	for (i = 1; i < npages_basemem; i++) {
		if(i!=mpg)
			mark_free(i);
	}
	uint32_t boot_alloc_end=PADDR(boot_alloc(0));
...
// if hpg==0 then use PGSIZE; else use HPGSIZE
static void
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	for(uint32_t i=0; i<size; i+=PGSIZE){
		pte_t* mpte=pgdir_walk(pgdir,(void*)(va+i),1);
		if(!mpte)
			panic("boot_map_region failed.");
		*mpte=(pa+i)|perm|PTE_P;
	}
}

// static void
// hboot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
// {
// 	// Fill this function in
// 	for(uint32_t i=0; i<size; i+=HPGSIZE){
// 		pte_t* mpte=&pgdir[PDX(va)];
// 		*mpte=(pa+i)|perm|PTE_P|PTE_PS;
...
{
	// Fill this function in
	pte_t* mpte=pgdir_walk(pgdir,va,1);
	if(!mpte)
		return -E_NO_MEM;
	pp->pp_ref++;
	if(*mpte&PTE_P)
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	pte_t* mpte;
	struct PageInfo* pp=page_lookup(pgdir,va,&mpte);
	if(pp){
		page_decref(pp);
		tlb_invalidate(pgdir,va);
		memset(mpte,0,sizeof(pte_t));
	}
}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	boot_map_region(kern_pgdir,base,rsize,pa,PTE_W|PTE_PWT|PTE_PCD);
	uintptr_t baset=base;
	base+=rsize;
	return (void*)baset;
}
...
check_va2pa(pde_t *pgdir, uintptr_t va)
{
	...
	if (!(*pgdir & PTE_P))
		return ~0;
	// if(large_page_enabled&&(*pgdir&PTE_PS)){
